...
def images_changed():
    ...
    if os.path.exists(ENCODINGS_FILE):
        ...
        print(f"folder : {folder_mod_time} - encodings : {encodings_mod_time}")
        
        # If the folder's modification time is older than the encodings file, return False
        if folder_mod_time <= encodings_mod_time:
            return False
    
    # If no encodings file exists or folder modification time is newer, return True
    return True
